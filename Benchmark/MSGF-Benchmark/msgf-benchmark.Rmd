...
library(mzID)
evalue = 1e-10
pnnl <- mzid_pnnl[, c("Scan", "MSGFDB_SpecEValue", "Peptide", "Protein")]
p3_flt <- flatten(mzid_p3)
bigmzid <- merge(pnnl, p3_table[, c("scan number(s)", "ms-gf:specevalue", "pepseq", "accession")], by.x="Scan", by.y = "scan number(s)", all.x = T)
```
...
### From Docker 
```{r echo=F}
tmp <- (p3_table[p3_table$'ms-gf:specevalue'<evalue & !is.na(p3_table$'ms-gf:specevalue'<evalue),c("scan number(s)", "ms-gf:specevalue", "pepseq", "accession")])
tmp <- tmp[order(tmp$`ms-gf:specevalue`),]
print("==Head==")
head(tmp,2)
print("==Tail==")
