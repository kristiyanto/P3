...
### Differences
Data from Docker is matched with from PNNL using scan number as ID. `r names(bt_tmp)[2:4]` are values from Docker, `r names(bt_tmp)[5:7]` are the corresponding values from PNNL. There are `r nrow(bt_tmp)` of rows in total with  `r sum(is.na(bt_tmp$MSGFDB_SpecEValue))` of NULL/unmapped values.

```{r, echo=F}
tmp_nona <- tmp[!is.na(tmp),]
```

Aside from the `r sum(is.na(bt_tmp$MSGFDB_SpecEValue))` unmatched values, `r sum(tmp_nona$'ms-gf:specevalue'!=tmp_nona$MSGFDB_SpecEValue)` unmacthed spec-evalue, `r sum(tmp_nona$pepseq!=tmp_nona$Peptide)` of unmatched peptide identification.

```{r echo=F}
print("==Head==")
head(bt_tmp,2)
print("==Unmatched==")
head(bt_tmp[is.na(bt_tmp$MSGFDB_SpecEValue),],3)
print("==Tail==")
tail(bt_tmp,2)
```
...
### From Docker 
```{r echo=F}
tmp <- (p3_table[!is.na(p3_table$'ms-gf:specevalue'<evalue),c("scan number(s)", "ms-gf:specevalue", "pepseq")])
tmp <- tmp[order(tmp$`ms-gf:specevalue`),]
print("==Head==")
head(tmp,2)
print("==Tail==")
...
bt_tmp <- merge(y =  tmp, x=pnnl, by.x = "Scan", by.y  =  "scan number(s)", all.x = T)
bt_tmp <- bt_tmp[order(bt_tmp$`ms-gf:specevalue`, decreasing = F),]
```
There are ```r nrow(tmp)``` rows remained.

### Differences
Data from Docker is matched with from PNNL using scan number as ID. `r names(bt_tmp)[2:3]` are values from Docker, `r names(bt_tmp)[4:5]` are the corresponding values from PNNL. There are `r nrow(bt_tmp)` of rows in total with  `r sum(is.na(bt_tmp$'ms-gf:specevalue'))` of NULL/unmapped values.

```{r, echo=F}
tmp_nona <- tmp[!is.na(tmp),]
```

Aside from the `r sum(is.na(bt_tmp$'ms-gf:specevalue'))` unmatched values, `r sum(tmp_nona$'ms-gf:specevalue'!=tmp_nona$MSGFDB_SpecEValue)` unmacthed spec-evalue, `r sum(tmp_nona$pepseq!=tmp_nona$Peptide)` of unmatched peptide identification.

```{r echo=F}
print("==Head==")
head(bt_tmp,2)
print("==Unmatched==")
head(bt_tmp[is.na(bt_tmp$'ms-gf:specevalue'),],3)
print("==Tail==")
tail(bt_tmp,2)
```
