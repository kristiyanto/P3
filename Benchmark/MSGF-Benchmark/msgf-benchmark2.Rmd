...
```
MSGF+ was run against `TCGA_13-1489_42-2590_36-2529_117C_W_PNNL_B2S5_f11_dta.txt` using the same command (10 ppm, without Docker). From `r nrow(dta)` rows, `r sum(duplicated(dta$Scan))` duplicated scan numbers were removed. 

```{r echo=F}
dta <- dta[,c("Scan","ms-gf:specevalue","pepseq")]
names(dta) <- c("Scan","MSGFDB_SpecEValue","Peptide_dta")
dta_merge <- merge(x=pnnl, y=dta, by=c("Scan","MSGFDB_SpecEValue"), all.x=T)

same      <- dta_merge[(dta_merge$Peptide_dta==dta_merge$Peptide),]
same      <- same[!is.na(same$Scan),]
print(paste("Exact match:", nrow(same),"out of", nrow(pnnl)))
      
diff_pep <- dta_merge[(dta_merge$Peptide_dta!=dta_merge$Peptide),]
print(paste("Diff peptide:", nrow(diff_pep)))
head(diff_pep)
```
