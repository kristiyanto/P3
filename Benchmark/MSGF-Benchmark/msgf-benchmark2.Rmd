...
```{r, warning=FALSE, echo=FALSE, include=FALSE, cache=F}
setwd("~/Dropbox/_SPACE/Benchmark/11")
...
### c. Differences
```{r echo=F}
m_table   <- merge(x=c_pnnl,y=docker,by="Scan", all=T)
...
tail(diff_spec,3)
diff_pep  <- m_table[(m_table$MSGFDB_SpecEValue!=m_table$Spec_Evalue) & (m_table$Peptide!=m_table$PepSeq),]
diff_pep  <- diff_pep[!is.na(diff_pep$Scan),]
print(paste("Different ID and Spec eValue:", nrow(diff_pep)))
tail(diff_pep,3)
```
...
setwd("~/Dropbox/_SPACE/Benchmark/11")
dta_mzid  <- mzID(file="TCGA_13-1489_42-2590_36-2529_117C_W_PNNL_B2S5_f11_msgfplus.mzid")
#dta_mzml  <- readMSData(files = file_mzml[2])
#dta       <- read.table("TCGA_13-1489_42-2590_36-2529_117C_W_PNNL_B2S5_f11_msgfdb.tsv", sep="\t", header = T)
dta       <- flatten(dta_mzid)
colnames(dta)[2] <- "Scan"

```
```{r echo=F}
dta_merge <- merge(x=dta[,c("Scan","ms-gf:specevalue","pepseq")], y=pnnl, by="Scan", all=T)

same      <- dta_merge[(dta_merge$MSGFDB_SpecEValue==dta_merge$'ms-gf:specevalue') & (dta_merge$Peptide==dta_merge$pepseq),]

same      <- same[!is.na(same$Scan),]
print(paste("Exact match:", nrow(same)))
print(paste("From Docker:", names(same)[2],names(same)[3],". From PNNL:", names(same)[4],names(same)[5]))
print(head(same[order(same$`ms-gf:specevalue`, decreasing = T),],2))
diff_spec      <- dta_merge[(dta_merge$MSGFDB_SpecEValue!=dta_merge$'ms-gf:specevalue') & (dta_merge$Peptide==dta_merge$pepseq),]

diff_spec <- diff_spec[!is.na(diff_spec$Scan),]
print(paste("Different Spec eValue:", nrow(diff_spec)))
tail(diff_spec[order(diff_spec$`ms-gf:specevalue`, decreasing = T),],3)
diff_pep      <- dta_merge[(dta_merge$MSGFDB_SpecEValue!=dta_merge$'ms-gf:specevalue') & (dta_merge$Peptide!=dta_merge$pepseq),]

diff_pep  <- diff_pep[!is.na(diff_pep$Scan),]
diff_scan <- nlevels(as.factor(diff_pep$Scan))
print(paste("Different ID and Spec eValue:", nrow(diff_pep), "with", diff_scan, "distict Scan numbers."))
tail(diff_pep[order(diff_pep$`ms-gf:specevalue`, decreasing = T),],3)
```
