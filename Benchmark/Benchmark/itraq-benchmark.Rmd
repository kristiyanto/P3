---
title: | 
  | Portable Proteomics Pipeline (P3) Benchmark
...
tmp[tmp$`scan number(s)`=="11612" &! is.na(tmp$`scan number(s)`),c("scan number(s)", "ms-gf:denovoscore", "ms-gf:specevalue", "accession", "pepseq")]
```

Reading the MZID file manually
```
java -Xmx2000M -XX:+UseConcMarkSweepGC -cp ../P3/MSGFPlus.jar edu.ucsd.msjava.ui.MzIDToTsv -i TCGA_13-1489_42-2590_36-2529_117C_W_PNNL_B2S5_f11_no_switch.mzid -showQValue 1 -showDecoy 1 -unroll 1
```
```{r, warning=FALSE, echo=FALSE, include=FALSE, cache=T}
setwd("~/Dropbox/_SPACE/Benchmark/")
